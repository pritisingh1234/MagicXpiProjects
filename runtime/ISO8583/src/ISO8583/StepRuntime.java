...
public class StepRuntime implements IStep {
	...
	public String logISOMsg(ISOMsg msg) throws IOException {
		String Response = "";
		...
		try {
			...
			for (int i = 1; i <= msg.getMaxField(); i++) {
				if (msg.hasField(i)) {
					...
					Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"msg.getString(i) Text "+msg.getString(i));
					Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"msg.getString(i) "+Arrays.toString(msg.getString(i).getBytes()));
				}
			}
	...
	public byte[] ISOTransactionReq(JSONObject object2, String ip, int port) {
		byte[] Response = null;
		ISOMsg reqMsg=null;
		StepRuntime srun=new StepRuntime();
		try{
			reqMsg=createISOMessage(object2);   
			Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Transaction_Request"+srun.logISOMsg(reqMsg));
				byte[] ss=reqMsg.pack();
				Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Step-reqMsg.pack() hexString: "+ISOUtil.hexString(reqMsg.pack()));
				Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Step-reqMsg.pack() hex2byte : "+new String(ISOUtil.hex2byte(new String(reqMsg.pack()))));
				Socket s=new Socket(ip,port);  
				DataOutputStream dout=new DataOutputStream(s.getOutputStream()); 
				Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Transaction_Request string conv"+ new String(reqMsg.pack()));
				//Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Transaction_Request string conv"+ new String(reqMsg.pack(),"UTF8"));
				//Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Transaction_Request string conv"+ new String(reqMsg.pack(),"ISO8859_1"));
				//Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Transaction_Request string conv"+ new String(reqMsg.pack(),"ISO_8859_1"));
				//Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Transaction_Request string conv"+ new String(reqMsg.pack(),"ISO-8859-1"));
				//dout.writeUTF(new String(reqMsg.pack())); 
				dout.writeUTF(ISOUtil.hexString(ss));
				dout.flush();  
				dout.close();  
				s.close();  
