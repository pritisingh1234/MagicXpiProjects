...
public class MgListener implements Runnable{
	...
	public void run() {
		...
		try{
				...
				String TranferRequest=dis.readUTF();  //original code
				//int TranferRequest1=dis.read();
				//String TranferRequest = Integer.toString(TranferRequest1);
				Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Transfer Request under MGListener: "+TranferRequest);
				Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Transfer Request under MGListener: "+ISOUtil.hex2byte(TranferRequest));
				...
				isoMsg.unpack(ISOUtil.hex2byte(TranferRequest));
				/*
