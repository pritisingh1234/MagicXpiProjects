...
public class Runtime implements IExternalTrigger{
	...
	void call(){
		...
		try {
	        ...
	        String f28= new SimpleDateFormat("yyMMdd").format(date);
	        ISOMsg reqMsg = SendReceiveClient.echoTest(object.getString ("MTI"),object.getString ("Header"), f7, serial, object.getString ("SignOn_Field24"), f28, object.getString ("Field93"), object.getString ("Field94"));
			
	        Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Sign ON ISO Request: "+logISOMsg(reqMsg));
			...
			if (asciiChnl.isConnected()) {
				...
				{
					Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Incoming Request SwitchData:\n"+ logISOMsg(respMsg));
					Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Incoming MyRequest SwitchData:\n"+ logISOMsg(respMsg).getBytes(StandardCharsets.UTF_8));
					Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Incoming MyRequest SwitchData:\n"+ logISOMsg(respMsg).getBytes());
					...
					args.put("SwitchData", logISOMsg(respMsg).getBytes());
						...
						if(result!=null)
						{
							...
							byte[] resdata=(byte[]) userP.getValue();
								//byte[] resdata=(byte[]) userP.getValue(s);
...
public  void echoTesting() {
	try {
		...
		ISOMsg reqMsg = SendReceiveClient.echoTest(object.getString ("MTI"),object.getString ("Header"), f7, object.getString ("Field11"), object.getString ("Field24"), f28, object.getString ("Field93"), object.getString ("Field94"));
