...
public class AsciiPrefixer implements Prefixer
{
	...
	public int decodeLength(byte[] b, int offset)
    {
        ...
        System.out.println("Recv Decode Length " + len );
        return len;
	...
	public int newDecodeLength(byte[] b, int offset)
    {
        ...
        int len = 0;
        System.out.println("Recv Printing ndigits " + nDigits);
        System.out.println("Recv Byte array " + new String(b));
        if( nDigits != 0)
        {
        ...
        System.out.println("Recv Diving length prefix by 2 " + newlens);
        if (newlens == 0)
        {
        	newlens = nDigits/2;
        	System.out.println("Prefix length is even " + newlens);
        	Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Prefix length is even " + newlens);
        }
        else
        {
        	newlens = (nDigits +1)/2;
        	System.out.println("Prefix length  is odd " + newlens);
        ...
        {
        	System.out.println("Recv Printing prefix as it is " + b[offset + 0]);
        	System.out.println("Recv Printing offset " + offset);
        	System.out.println("Recv Printing i "+ i);
        	System.out.println("Recv Printing data at position array " + ISOUtil.hexString(b));
        	...
        	System.out.println("Recv array value in String ");
        	
        //	String Text= new String(b, StandardCharsets.US_ASCII);
        	
        //	System.out.println("Recv String Value for String " + Text);

        	
        	//String c = String.valueOf(b[offset -1]);
        	//System.out.println("Rec Integer value " + c);
        	//String d =String.valueOf(c);
        	//char ch = c.charAt(0) ;
        	//System.out.println("Recv char value at that pos " + ch);
        	//Integer lenV = (int)(ch);
        	//System.out.println("String to dec convertion " + lenV);
