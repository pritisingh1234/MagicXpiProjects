...
public class ISOStringFieldPackager extends ISOFieldPackager
{
	...
	public byte[] pack(ISOComponent c) throws ISOException
    {
        try
        {
            ...
            Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,hlen);
            Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,""+Integer.parseInt(hlen));
            Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,Integer.toHexString(Integer.parseInt(hlen)));
            Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,Integer.toHexString(Integer.parseInt(hlen)));
            Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,new String(rawData).substring(prefixer.getPackedLength(),Integer.parseInt(hlen)+prefixer.getPackedLength()));
            String strLen=new String(Integer.toHexString(Integer.parseInt(hlen)));
            Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Actual Hlen:"+new String(Integer.toHexString(Integer.parseInt(hlen))));
            ...
            if(hlen1<prefixer.getPackedLength())
            {
            	...
            	Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Length in Hex to Ascii" + new String (ISOUtil.hex2byte(new String(Integer.toHexString(Integer.parseInt(hlen))))) );
            	Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Data in Hex to Ascii" + new String (ISOUtil.hex2byte(new String(rawData).substring(prefixer.getPackedLength(),Integer.parseInt(hlen)+prefixer.getPackedLength()))));
            ...
            if (fnum == 103 || fnum == 22 || fnum == 37 || fnum == 41 || fnum == 42 ||  fnum == 43 ||  fnum == 54 || fnum == 102 || fnum == 103)
            {
            	/*
            ...
            else
            {
            	 ...
            	 */
            	//rawData = new String (ISOUtil.hex2byte(new String(rawData))).getBytes() ;
            	byte[] rawAscii=ISOUtil.hex2byte(new String(rawData));
            	rawData =rawAscii ;
            	Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"rawData [HHH] - "+new String(rawData));
            	Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"rawData [RRR3] - "+ISOUtil.hexString(rawData));
            }
	...
	public int unpack(ISOComponent c, byte[] b, int offset) throws ISOException
    {
        try
        {
        	Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module, "[8583]Incomming ISO request byte array "+Arrays.toString(b));
        	Integer fnum = (Integer)c.getKey();
        	Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Field number " + fnum);
        	Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Field number " + fnum);
        	Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Recv Inside ISOStringFieldPackager Unpack function");
        	...
        	Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Recv incoming byte array hex" + ISOUtil.hexString(b));
            ...
            Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Recv Length of Prefix after division by 2 " + lenLen);
            }
            ...
            if (fnum == 103 || fnum == 22 || fnum == 37 || fnum == 41 || fnum == 42 ||   fnum == 43 || fnum == 54 || fnum == 102 || fnum == 103)
            {
            	...
            	Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Recv The field is of CHar type with length " + len);
            	...
            	// unpacked = unpacked.substring(difference, origLen);
            	 ...
            	 Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Unpacked data " + unpacked);
            ...
            unpacked = ISOUtil.hexString(interpreter.uninterpret(b, offset + lenLen, len).getBytes());
            Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Unpacked data " + unpacked);
