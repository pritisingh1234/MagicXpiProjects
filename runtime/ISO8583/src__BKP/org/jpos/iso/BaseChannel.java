               ...
               LogSource, Configurable, BaseChannelMBean, Cloneable
{
    ...
    */
    ...
    static String logISOMsg(ISOMsg msg) throws IOException {
		...
		com.magicsoftware.ibolt.commons.logging.Logger.logMessage(LogLevel.LEVEL_DEBUG, "",""+Response);
		return Response;
	}
    /**
	...
	public ISOMsg receive() throws IOException, ISOException {
        ...
        com.magicsoftware.ibolt.commons.logging.Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module," m \n"+logISOMsg(m));
        m.setSource (this);
        try {
            if (!isConnected())
                throw new IOException ("unconnected ISOChannel");
            ...
            synchronized (serverInLock) {
               ...
               // m.setHeader("ISO8583-1993001000000".getBytes());
                int hLen = getHeaderLength();
                //Checking the value by  settng header leagnth as 21
                
               hLen = 21;
                
                com.magicsoftware.ibolt.commons.logging.Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Recv Header Length" + getHeaderLength());
                ...
                if (len == -1) {
                    ...
                    if (hLen > 0) {
                        ...
                        header = readHeader(hLen);
                        len -= header.length;
                        com.magicsoftware.ibolt.commons.logging.Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Recv Length minus header" + len);
                    }
                    b = new byte[len];
                    com.magicsoftware.ibolt.commons.logging.Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Recv Incoming Byte for the length without header" + new String(b));
                    getMessage (b, 0, len);
                    com.magicsoftware.ibolt.commons.logging.Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Recv Incoming Byte Array 2" + new String(b));
                    getMessageTrailer(m);
            ...
            {
            	com.magicsoftware.ibolt.commons.logging.Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Recv Calling Unpack Function" );
                unpack (m, b);
            }
            m.setDirection(ISOMsg.INCOMING);
            evt.addMessage (m);
            com.magicsoftware.ibolt.commons.logging.Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Recv Printing Evt message"  );
    ...
    protected void unpack (ISOMsg m, byte[] b) throws ISOException {
    	com.magicsoftware.ibolt.commons.logging.Logger.logMessage(LogLevel.LEVEL_DEBUG, this.module,"Recv Calling unpack from Base Channel 2");
    	m.unpack (b);
